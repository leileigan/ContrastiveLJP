...
from models.model_NeurJudge_moco_hmce import MoCo
...
def evaluate(model, valid_dataloader, process, name, epoch_idx):
    ...
    for batch_idx, datapoint in enumerate(valid_dataloader):
        documents, accu_label_lists, law_label_lists, term_lists = datapoint
        sent_lent = ""
...
def train(model, dataset, config: Config):
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx, datapoint in enumerate(tqdm(train_dataloader)):
            ...
            loss = (accu_loss + term_loss + law_loss) / batch_size + config.alpha * 0 #contra_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default="./datasets/NeurJudge/")
    parser.add_argument('--status', default="train")
    parser.add_argument('--savemodel', default="./results/NeurJudge/NeurJudge_moco_hmce")
    parser.add_argument('--loadmodel', default="")

    parser.add_argument('--embedding_path', default='./data/cail_thulac.npy')
    parser.add_argument('--word2id_dict', default='./data/w2id_thulac.pkl')
    ...
    if status == 'train':
        ...
        train_dataset = NeurJudgeDataset(train_data, tokenizer, config.MAX_SENTENCE_LENGTH)
        valid_dataset = NeurJudgeDataset(valid_data, tokenizer, config.MAX_SENTENCE_LENGTH)
        test_dataset = NeurJudgeDataset(test_data, tokenizer, config.MAX_SENTENCE_LENGTH)
