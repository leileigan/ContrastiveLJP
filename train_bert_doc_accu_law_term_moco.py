...
from models.model_BERT_doc_accu_law_term_moco import MoCo
...
class BERTDataset(Dataset):
    ...
    def collate_bert_fn(self, batch):
        ...
        return padded_input_ids, padded_token_type_ids, padded_attention_mask, batch_raw_fact_list, padded_accu_label_lists, padded_law_label_lists, padded_term_lists
...
def evaluate(model, valid_dataloader, name, epoch_idx):
    ...
    for batch_idx, datapoint in enumerate(valid_dataloader):
        fact_list, type_ids_list, attention_mask_list, _, accu_label_lists, law_label_lists, term_lists = datapoint
        accu_preds, law_preds, term_preds = model.predict(fact_list, type_ids_list, attention_mask_list, accu_label_lists,law_label_lists, term_lists)
...
def train(model, dataset, config: Config):
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx, datapoint in enumerate(train_dataloader):
            fact_list, type_ids_list, attention_mask_list, _, accu_label_lists, law_label_lists, term_lists = datapoint
            contra_doc_loss, contra_accu_loss, contra_law_loss, contra_term_loss, accu_loss, law_loss, term_loss, accu_preds, law_preds, term_preds = model.forward(fact_list, type_ids_list, attention_mask_list,accu_label_lists, law_label_lists, term_lists)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savemodel', default=BASE+"/results/bert/XsBERT_doc_accu_law_term_moco")
