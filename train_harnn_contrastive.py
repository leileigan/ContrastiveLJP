...
def evaluate(model, valid_dataloader, name):
    ...
    for batch_idx, datapoint in enumerate(valid_dataloader):
        ...
        _, _, _, _, accu_preds, law_preds, term_preds = model.neg_log_likelihood_loss(fact_list, accu_label_lists,law_label_lists, term_lists, config.sent_len, config.doc_len)
...
def train(model, dataset, config: Config):
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx, datapoint in enumerate(train_dataloader):
            fact_list, accu_label_lists, law_label_lists, term_lists = datapoint
            accu_loss, law_loss, term_loss, cl_loss, accu_preds, law_preds, term_preds = model.neg_log_likelihood_loss(fact_list, 
                                                                                                              accu_label_lists, 
                                                                                                              law_label_lists, 
                                                                                                              term_lists, 
                                                                                                              config.sent_len, 
            ...
            loss = (accu_loss + term_loss + law_loss) / batch_size + 0.1 * cl_loss
            ...
            if (batch_idx + 1 ) % 100 == 0:
                ...
                print("Instance: %s; Time: %.2fs; loss: %.4f; accu loss %.4f; law loss %.4f; term loss %.4f; cl loss %.4f; accu acc %.4f; law acc %.4f; term acc %.4f" % 
