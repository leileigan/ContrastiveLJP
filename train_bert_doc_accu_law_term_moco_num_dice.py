...
from models.model_BERT_doc_accu_law_term_moco_num_dice import MoCo
...
class BERTDataset(Dataset):
    ...
    def collate_bert_fn(self, batch):
        ...
        batch_out = self.tokenizer.batch_encode_plus(batch_raw_fact_list, max_length=500, padding='max_length', return_tensors='pt', truncation=True)
        ...
        return padded_input_ids, padded_token_type_ids, padded_attention_mask, batch_raw_fact_list, padded_accu_label_lists, padded_law_label_lists, padded_term_lists, padded_money_amount_lists, padded_drug_weight_lists
...
def evaluate(model, valid_dataloader, name, epoch_idx):
    ...
    for batch_idx, datapoint in enumerate(valid_dataloader):
        fact_list, type_ids_list, attention_mask_list, _, accu_label_lists, law_label_lists, term_lists, money_lists, drug_lists = datapoint
        accu_preds, law_preds, term_preds = model.predict(fact_list, type_ids_list, attention_mask_list, accu_label_lists,law_label_lists, term_lists, money_lists, drug_lists)
...
def train(model, data_dict, config: Config):
    train_dataloader = data_dict["train_data_set"]
    # train_data_set = dataset["valid_data_set"]
    valid_dataloader = data_dict["valid_data_set"]
    test_dataloader = data_dict["test_data_set"]
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx, datapoint in enumerate(train_dataloader):
            fact_list, type_ids_list, attention_mask_list, _, accu_label_lists, law_label_lists, term_lists , money_amount_lists, drug_lists = datapoint
            contra_doc_loss, contra_accu_loss, contra_law_loss, contra_term_loss, accu_loss, law_loss, term_loss, accu_preds, law_preds, term_preds = model.forward(fact_list, type_ids_list, attention_mask_list,accu_label_lists, law_label_lists, term_lists, money_amount_lists, drug_lists)

            loss = accu_loss + term_loss + law_loss + config.alpha1 * contra_accu_loss + config.alpha2 * contra_law_loss + config.alpha3 * contra_term_loss + config.alpha4 * contra_doc_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savemodel', default=BASE+"/results/bert/XsBERT_doc_accu_law_term_moco_num_dice")
    ...
    if status == 'train':
        ...
        valid_dataloader = DataLoader(valid_dataset, batch_size=config.HP_batch_size, shuffle=False, collate_fn=train_dataset.collate_bert_fn)
        test_dataloader = DataLoader(test_dataset, batch_size=config.HP_batch_size, shuffle=False, collate_fn=train_dataset.collate_bert_fn)
        ...
        print("train_data %d, valid_data %d, test_data %d." % (
            len(data_dict['train_data_set'].dataset), len(data_dict['valid_data_set'].dataset), len(data_dict['test_data_set'].dataset)))
        ...
        train_dataloader = DataLoader(train_dataset, batch_size=config.HP_batch_size, shuffle=False, collate_fn=train_dataset.collate_bert_fn)
        valid_dataloader = DataLoader(valid_dataset, batch_size=config.HP_batch_size, shuffle=False, collate_fn=train_dataset.collate_bert_fn)
        test_dataloader = DataLoader(test_dataset, batch_size=config.HP_batch_size, shuffle=False, collate_fn=train_dataset.collate_bert_fn)
