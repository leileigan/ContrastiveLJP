...
import pickle
from models.model_BERT import LawModel
import sys, json
...
# doc_words, claims, doc_word_ids, sentences_num, doc_sentence_lens, claims_ids, claims_num, claims_len, fact_labels, claims_labels
    doc_words = []
    doc_word_ids = []
    fact_labels = []
    claims_labels = []
    ...
    for item in data:
        for claim_text in item[1]:
            new_doc_words = item[0]
            if len(new_doc_words) > 1500:
                new_doc_words = new_doc_words[:1500]

            new_doc_words += claim_text
            doc_words.append(new_doc_words)


        for claim_id in item[5]:
            new_dialogue_word_id = item[2]
            if len(new_dialogue_word_id) > 1500:
                new_dialogue_word_id = new_dialogue_word_id[:1500]
            new_dialogue_word_id += claim_id
            doc_word_ids.append(new_dialogue_word_id)

            doc_lens.append(len(new_dialogue_word_id))
            fact_labels.append(item[8])

        claims_labels.append(item[9])
    ...
    data = list(zip(doc_words, doc_word_ids, doc_lens, fact_labels, claims_labels))
    return data
...
def load_data(path, config: Data):
    ...
    def _do_vectorize(jsonContent):
        ...
        for key in keys:
            ...
            for claim in claims_split:
                ...
                claims_text.append(claim_list)
                claims_id = [config.word_alphabet.get_index(word) for word in claim_list]
                claims_sequences.append(claims_id)

            claims.append(claims_text)
            claims_ids.append(claims_sequences)
            ...
            doc_words.append(chaming_list)
            chaming_ids = [config.word_alphabet.get_index(word) for word in chaming_list]
            doc_words_ids.append(chaming_ids)
            sentence_nums.append(len(chaming.split('\n')))
            doc_sentences_lens.append(len(chaming_list))


        return doc_words, claims, doc_words_ids, sentence_nums, doc_sentences_lens, claims_ids, claims_num, claims_len
    ...
    # encode x
    doc_words, claims, doc_word_ids, sentences_num, doc_sentence_lens, claims_ids, claims_num, claims_len = _do_vectorize(jsonContent)
    fact_labels, claims_labels = _do_label_vectorize(jsonContent)
    data = list(zip(doc_words, claims, doc_word_ids, sentences_num, doc_sentence_lens,  claims_ids, claims_num, claims_len, fact_labels, claims_labels))
    return data
...
def generate_batch_instance(input_data, gpu):
    # input_data = list(zip(doc_words, doc_word_ids, doc_lens, fact_labels, claims_labels))
    ...
    word_text = [item[0] for item in input_data]
    word_seq_tensor = [torch.LongTensor(item[1]) for item in input_data] # batch_size, max_sequence_length
    word_seq_tensor = pad_sequence(word_seq_tensor, batch_first=True, padding_value=0)

    fact_labels_tensor = torch.FloatTensor([item[3] for item in input_data]) # batch_size, fact_num
    claims_labels_tensor = torch.FloatTensor([item[4] for item in input_data]) # batch_size

    sentence_lens = [item[2] for item in input_data]
    ...
    return word_text, word_seq_tensor, claims_labels_tensor, fact_labels_tensor, sentence_lens
...
def evaluate(model, dataset, name):
    ...
    for batch_idx in range(total_batch):
        ...
        word_text_batch, input_x_batch, claims_labels_batch, fact_labels_batch, input_sentences_len_batch \
            = generate_batch_instance(batch_instances, config.HP_gpu)
        try:
            claim_loss,  batch_claim_predicts = model.neg_log_likelihood_loss(
            word_text_batch, input_x_batch, input_sentences_len_batch, fact_labels_batch, claims_labels_batch)
            loss = claim_loss
...
def train(dataset, config: Data):
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx in range(total_batch):
            ...
            claim_loss, batch_claim_predicts = model.neg_log_likelihood_loss(word_text_batch,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', default="./data/chaming-dev.json")
    ...
    parser.add_argument('--HP_batch_size', default=1)
    ...
    parser.add_argument('--HP_lr', default=5e-5)
