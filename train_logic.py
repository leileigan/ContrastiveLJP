...
import pickle
from models.model_Logic import LawModel
import sys, json
...
def get_claim_type(claim):
    if ("借款" in claim or "本金" in claim) and "利息" not in claim and "违约" not in claim \
                and "担保" not in claim:
        claim_type = '本金'
...
def load_data(path, config: Data):
    ...
    def _do_vectorize(jsonContent):
        ...
        for key in keys:
            ...
            for claim in claims_split:
                ...
                cur_claim_type = get_claim_type(claim)
                type.append(cur_claim_type)
                claim_type_dic[cur_claim_type] = claim_type_dic[cur_claim_type] + 1 if cur_claim_type in claim_type_dic else 1


            claims_len.append([len(x) for x in claims_sequences])
            claims.append(claims_sequences)
            claims_num.append(len(claims_split))
            claims_type.append(type)
        ...
        return doc_words_ids, sentence_nums, doc_sentences_lens, claims, claims_num, claims_len, claims_type
    ...
    # encode x
    doc_word_ids, sentences_num, doc_sentence_lens, claims, claims_num, claims_len, claims_type = _do_vectorize(jsonContent)
    fact_labels, claims_labels = _do_label_vectorize(jsonContent)
    data = list(zip(doc_word_ids, doc_sentence_lens,  claims, claims_num, claims_len, claims_type,
                    fact_labels, claims_labels))
    return data
...
def generate_batch_instance(input_data, gpu):
    ...
    claims_type = [item[4] for item in input_data]
    ...
    return word_seq_tensor, claims_labels_tensor, fact_labels_tensor, sentence_lens, claims_type
...
def train(dataset, config: Data):
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx in range(total_batch):
            ...
            if len(batch_instances) <= 0:
                continue
            input_x_batch, claims_labels_batch, fact_labels_batch, input_sentences_len_batch, claims_type_batch \
                = generate_batch_instance(batch_instances, config.HP_gpu)

            claim_loss, fact_loss, batch_fact_predicts, batch_claim_predicts = model.neg_log_likelihood_loss(
                    input_x_batch, input_sentences_len_batch, fact_labels_batch, claims_labels_batch, claims_type_batch)
