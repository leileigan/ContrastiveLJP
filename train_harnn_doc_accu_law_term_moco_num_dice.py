...
from models.model_HARNN_doc_accu_law_term_moco_num_dice import MoCo
...
def evaluate(model, valid_dataloader, name, epoch_idx):
    ...
    for batch_idx, datapoint in enumerate(valid_dataloader):
        ...
        _, _, _, _, _, _, _, accu_preds, law_preds, term_preds = model.forward(fact_list, accu_label_lists,law_label_lists, term_lists, config.sent_len, config.doc_len, money_lists, drug_lists)
...
def train(model, dataset, config: Config):
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx, datapoint in enumerate(train_dataloader):
            ...
            contra_doc_loss, contra_accu_loss, contra_law_loss, contra_term_loss, accu_loss, law_loss, term_loss, accu_preds, law_preds, term_preds  = model.forward(fact_list, accu_label_lists, law_label_lists, term_lists, config.sent_len, config.doc_len, money_amount_lists, drug_lists)
