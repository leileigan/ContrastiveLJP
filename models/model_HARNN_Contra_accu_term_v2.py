...
class MoCo(nn.Module):
    ...
    @torch.no_grad()
    def _dequeue_and_enqueue(self, keys, accu_label_lists, term_label_lists):
        ...
        self.term_label_queue[ptr:ptr+batch_size, :] = term_label_lists.unsqueeze(1)
        ptr = (ptr + batch_size) % self.K  # move pointer
        self.queue_ptr[0] = ptr
        self.label_ptr[0] = ptr


    def _get_contra_loss(self, query, accu_label_lists, term_label_lists):
        ...
        # contrastive loss for term and charge:
        # positive examples: examples from the sample charge class and term class
        # negative examples: examples from different charge classes
        neg_mask = accu_term_mask + (1 - accu_mask.float()) #[bsz, queue_size] # positive examples and negative examples
        log_prob = query_queue_product - torch.log((exp_logits * neg_mask).sum(1, keepdim=True) + 1e-12) #[bsz, queue_size]
        mean_log_prob_pos_term_acc = (accu_term_mask * log_prob).sum(1) / (accu_mask.sum(1) + 1e-12)
        loss_accu_term = -mean_log_prob_pos_term_acc.mean()

        # contrastive loss for charge:
        # examples from the same charge class are considered as positive examples
        # examples from different term class are considered as negative examples
        # log_prob = query_queue_product - torch.log(exp_logits.sum(1, keepdim=True) + 1e-12) #[bsz, queue_size]
        # mean_log_prob_pos_acc = (accu_mask * log_prob).sum(1) / (accu_mask.sum(1) + 1e-12)
        # loss_accu = -mean_log_prob_pos_acc.mean()

        return loss_accu_term
    ...
    def forward(self, fact_list, accu_label_lists, law_label_lists, term_lists):
        ...
        # dequeue and enqueue
        self._dequeue_and_enqueue(k, accu_label_lists, term_lists)
        contra_loss = self._get_contra_loss(q, accu_label_lists, term_lists)
