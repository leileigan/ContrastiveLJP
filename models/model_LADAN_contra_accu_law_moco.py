...
def dynamic_partition(data, partitions, num_partitions):
    ...
    for i in range(num_partitions):
        ...
        if len(data[torch.LongTensor(partitions).eq(i).nonzero(as_tuple=False).squeeze().tolist()].size()) < 2:
            res.append(data[torch.LongTensor(partitions).eq(
                i).nonzero(as_tuple=False).squeeze().tolist()].unsqueeze(0))
        else:
            res.append(data[torch.LongTensor(partitions).eq(
                i).nonzero(as_tuple=False).squeeze().tolist()])
    return res
...
class LawModel(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.word_embeddings_layer = torch.nn.Embedding(
            config.pretrain_word_embedding.shape[0], config.pretrain_word_embedding.shape[1], padding_idx=0)
        ...
        self.lstm_layer1 = nn.LSTM(self.word_dim, hidden_size=self.hidden_dim,
                                   num_layers=1, batch_first=True, bidirectional=True)  # word lstm
        self.lstm_layer2 = nn.LSTM(self.hidden_dim * 2, hidden_size=self.hidden_dim,
                                   num_layers=1, batch_first=True, bidirectional=True)  # sentence lstm
        ...
        self.fully_attn_sent_1 = torch.nn.Linear(
            self.hidden_dim*2, self.hidden_dim*2)
        self.fully_attn_doc_1 = torch.nn.Linear(
            self.hidden_dim*2, self.hidden_dim*2)
        ...
        # init graph
        self.law_input, graph_list_1, graph_membership, neigh_index = get_law_graph(
            self.law_relation_threshold, config.word2id_dict, 15, 100)
        ...
        # re-encoder
        self.re_encoder_linear_1 = torch.nn.Linear(
            4*self.hidden_dim, 2*self.hidden_dim)
        self.re_encoder_linear_2 = torch.nn.Linear(
            4*self.hidden_dim, 2*self.hidden_dim)
        self.lstm_layer3 = nn.LSTM(self.hidden_dim * 2, hidden_size=self.hidden_dim,
                                   num_layers=1, batch_first=True, bidirectional=True)  # sentence lstm

        self.graph_classifier = torch.nn.Linear(
            self.hidden_dim * 2, self.max_graph)
        ...
        self.fully_attn_sent_2 = torch.nn.Linear(
            self.hidden_dim*2, self.hidden_dim*2)
        self.fully_attn_doc_2 = torch.nn.Linear(
            self.hidden_dim*2, self.hidden_dim*2)
    ...
    def classifier_layer(self, doc_out, law_out, accu_labels, law_labels, term_labels, law_article_labels):
        ...
        law_article_loss = self.law_article_loss(
            law_article_log_softmax, law_article_labels)
        _, law_article_preds = torch.max(law_article_log_softmax, dim=-1)

        accu_logits = self.accu_classifier(
            doc_out)  # [batch_size, accu_label_size]
        accu_log_softmax = F.log_softmax(accu_logits, dim=-1)
        accu_loss = self.accu_loss(accu_log_softmax, accu_labels)
        # [batch_size, accu_label_size]
        _, accu_predicts = torch.max(accu_log_softmax, dim=-1)

        # [batch_size, law_label_size]
        law_logits = self.law_classifier(doc_out)
        law_log_softmax = F.log_softmax(law_logits, dim=-1)
        law_loss = self.law_loss(law_log_softmax, law_labels)
        # [batch_size * max_claims_num]
        _, law_predicts = torch.max(law_log_softmax, dim=1)

        term_logits = self.term_classifier(
            doc_out)  # [batch_size, term_label_size]
        term_log_softmax = F.log_softmax(term_logits, dim=-1)
        term_loss = self.term_loss(term_log_softmax, term_labels)
        # [batch_size * max_claims_num]
        _, term_predicts = torch.max(term_log_softmax, dim=1)
    ...
    def word_attn_lstm(self, input: torch.FloatTensor, mask: torch.BoolTensor):
        ...
        hidden = None
        # [batch_size*max_sent_num, max_sent_len, hidden_dim * 2]
        lstm1_out, hidden = self.lstm_layer1.forward(input, hidden)
        # [batch_size*max_sent_num, max_sent_len, 1]
        attn_p_weights = torch.matmul(lstm1_out, self.attn_p)
        # [batch_size * max_sent_num, max_sent_len]
        attn_p_outs = F.softmax(attn_p_weights.squeeze()
                                * mask, dim=-1).unsqueeze(-1)
        # [batch_size, max_sent_num, hidden_dim*2]
        sent_rep = torch.sum(lstm1_out * attn_p_outs,
                             dim=1).view(batch_size, max_sent_num, -1)
        return sent_rep
    ...
    def sent_attn_lstm(self, input: torch.FloatTensor, mask: torch.BoolTensor):
        ...
        hidden = None
        # [batch_size, max_sent_num, hidden_dim * 2]
        lstm2_out, hidden = self.lstm_layer2.forward(input, hidden)
        # [batch_size, max_sent_num, 1]
        attn_q_weights = torch.matmul(lstm2_out, self.attn_q)
        attn_q_outs = F.softmax(attn_q_weights.squeeze()
                                * mask, dim=-1).unsqueeze(-1)
        # [batch_size, hidden_dim*2]
        doc_rep = torch.sum(lstm2_out * attn_q_outs, dim=1)
        return doc_rep
    ...
    def GDO(self, law_rep_partition: List[torch.FloatTensor], law_rep: torch.FloatTensor, indices):
        ...
        for i in range(1, self.max_deg+1):
            ...
            neigh_articles = torch.index_select(input=law_rep, dim=0, index=torch.LongTensor(
                list(chain(*self.adj_list[i]))).cuda())  # [number_of_degree_i, degree, hidden_size]
            ...
            # [number_of_degree_i, degree, 2*hidden_size]
            interaction_vec = torch.cat((article_1, neigh_articles), dim=-1)
            # [number_of_degree_i, 2*hidden_size]
            neigh_articles = torch.mean(
                self.gdo_linear(interaction_vec), dim=1)
            # [number_of_degree_i, 2*hidden_size]
            new_article = self.gdo_mlp(article-neigh_articles)
    ...
    def law_re_encoder(self, atten_list, law_sent_rep, law_mask, law_doc_mask):
        ...
        law_u = torch.index_select(input=torch.stack(atten_list), dim=0, index=torch.LongTensor(
            self.graph_list).cuda())  # [num_of_law_articles, hidden_dim]
        # print("law sent rep:", law_sent_rep.size())
        # print("law mask size:", law_mask.size())

        # [num_of_law_articles*max_law_sent_num, max_law_sent_len, hidden_dim]
        u_law_w = self.re_encoder_linear_1(law_u).unsqueeze(
            1).repeat(max_law_sent_num, max_law_sent_len, 1)
        # [num_of_law_articles, max_law_sent_num, hidden_dim]
        u_law_s = self.re_encoder_linear_2(law_u).unsqueeze(
            1).expand(-1, max_law_sent_num, -1)
        ...
        rep_law, _ = attn_encoder_mask(
            u_law_w, law_sent_rep, self.fully_attn_sent_2, law_mask, k_ori=True)
        # print("rep law size:", rep_law.size())
        rep_law = rep_law.view(-1, max_law_sent_num, 2*self.hidden_dim)

        hidden = None
        # [num_of_law_articles, max_law_sent_num, hidden_dim]
        rep_law_2, _ = self.lstm_layer3(rep_law, hidden)
        # attend original law_doc_rep to law graph community representations
        rep_law_2, _ = attn_encoder_mask(
            u_law_s, rep_law_2, self.fully_attn_doc_2, law_doc_mask, k_ori=True)
    ...
    def fact_re_encoder(self,
        ...
        fact_graph_choose_1 = self.graph_classifier(
            fact_basic_doc_rep)  # [batch_size, 68]
        fact_graph_choose = F.softmax(
            fact_graph_choose_1, dim=-1)  # [batch_size, 68]
        ...
        graph_choose_loss = self.law_graph_community_loss(
            graph_choose_log_softmax, graph_community_labels)
        # print("graph choose loss:", graph_choose_loss)
        graph_choose_loss = graph_choose_loss / 128.0
        _, graph_preds = torch.max(
            graph_choose_log_softmax, dim=-1)  # [batch_size]
        ...
        u_fact = torch.matmul(fact_graph_choose, atten_tensor)
        u_fact_w = self.re_encoder_linear_1(u_fact).unsqueeze(
            1).repeat(max_fact_sent_num, max_fact_sent_len, 1)
        u_fact_s = self.re_encoder_linear_2(u_fact).unsqueeze(
            1).expand(-1, max_fact_sent_num, -1)
        fact_mask = fact_mask.view(-1, max_fact_sent_len)
        rep_fact, _ = attn_encoder_mask(
            u_fact_w, fact_lstm_sent_rep, self.fully_attn_sent_2, fact_mask, k_ori=True)
        ...
        rep_fact_2, _ = attn_encoder_mask(
            u_fact_s, rep_fact_2, self.fully_attn_doc_2, fact_doc_mask, k_ori=True)
    ...
    def forward(self, input_facts, accu_labels, law_labels, term_labels):
        ...
        gold_matrix_law = F.one_hot(
            law_labels, 103).float()  # [batch_size, 103]
        gold_matrix_accu = F.one_hot(
            accu_labels, 119).float()  # [batch_size, 119]
        gold_matrix_time = F.one_hot(
            term_labels, 12).float()  # [batch_size, 12]
        ...
        # [batch_size, max_sent_num, max_sent_seq_len] -> [batch_size, max_sent_num]
        fact_sent_len = torch.sum(fact_mask, dim=-1)
        fact_doc_mask = fact_sent_len.bool()
        # [batch_size, max_sent_num] -> [batch_size]
        fact_doc_len = torch.sum(fact_doc_mask, dim=-1)
        ...
        # [batch_size*max_sent_num, max_doc_seq_len, word_emb_size]
        fact_word_embeds = self.word_embeddings_layer(
            input_facts.view(batch_size*max_fact_sent_num, -1))
        law_word_embeds = self.word_embeddings_layer(
            self.law_input.view(self.config.law_label_size*max_law_sent_num, -1))

        # basic hrnn encoder
        hidden = None
        # [batch_size*max_sent_num, max_sent_len, hidden_dim * 2]
        fact_lstm_sent_out, hidden = self.lstm_layer1.forward(
            fact_word_embeds, hidden)
        fact_sent_rep, _ = attn_encoder_mask(
            self.attn_p, fact_lstm_sent_out, self.fully_attn_sent_1, fact_mask.view(-1, max_fact_sent_seq_len), k_ori=True)
        hidden = None
        # [batch_size*max_sent_num, max_sent_len, hidden_dim * 2]
        law_lstm_sent_out, hidden = self.lstm_layer1.forward(
            law_word_embeds, hidden)
        law_sent_rep, _ = attn_encoder_mask(
            self.attn_p, law_lstm_sent_out, self.fully_attn_sent_1, law_mask.view(-1, max_law_sent_seq_len), k_ori=True)

        hidden = None
        fact_lstm_doc_out, hidden = self.lstm_layer2.forward(fact_sent_rep.view(
            batch_size, max_fact_sent_num, -1), hidden)  # [batch_size, max_sent_num, hidden_dim * 2]
        fact_doc_rep, _ = attn_encoder_mask(
            self.attn_q, fact_lstm_doc_out, self.fully_attn_doc_1, fact_doc_mask, k_ori=True)  # [batch_size, hidden_size]
        hidden = None
        # [num_of_law_articles, max_sent_num, hidden_dim * 2]
        law_lstm_doc_out, hidden = self.lstm_layer2.forward(
            law_sent_rep.view(-1, max_law_sent_num, 2*self.hidden_dim), hidden)
        # [num_of_law_articles, hidden_size]
        law_doc_rep, _ = attn_encoder_mask(
            self.attn_q, law_lstm_doc_out, self.fully_attn_doc_1, law_doc_mask, k_ori=True)

        # graph interaction
        indices = dynamic_partition(torch.LongTensor(range(
            self.config.law_label_size)), self.deg_list, self.max_deg + 1)  # clustering laws w.r.t their degrees
        # a list of [number_of_degrees, hidden_size]
        law_rep_of_degrees = dynamic_partition(
            law_doc_rep, self.deg_list, self.max_deg + 1)
        ...
        law_rep_of_degrees = dynamic_partition(
            sec_law_conv, self.graph_list, self.max_graph)
        atten_list = []
        for i in range(self.max_graph):
            ...
            # size: [graph_num, 4*hidden_size] whether this u can use attention to get
            atten_list.append(torch.cat([u_max, u_min], -1))

        law_re_rep = self.law_re_encoder(
            atten_list, law_lstm_sent_out, law_mask, law_doc_mask)
        law_final_rep = torch.cat((law_doc_rep, law_re_rep), dim=-1)
        fact_re_rep, graph_choose_loss, graph_preds = self.fact_re_encoder(
            atten_list, fact_doc_rep, fact_lstm_sent_out, graph_labels, fact_mask, fact_doc_mask)
        fact_final_rep = torch.cat((fact_doc_rep, fact_re_rep), dim=-1)
        accu_preds, accu_loss, law_preds, law_loss, term_preds, term_loss, law_article_preds, law_article_loss = self.classifier_layer(
            fact_final_rep, law_final_rep, accu_labels, law_labels, term_labels, law_article_labels)  # [batch_size, 3]
        return fact_final_rep, accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds
...
class MoCo(nn.Module):
    ...
    def _get_contra_loss(self, query, accu_label_lists, law_label_lists, term_label_lists):
        ...
        cos_sim = query_queue_product / torch.einsum('nc,kc->nk', [torch.norm(query, dim=1).unsqueeze(
            1), torch.norm(self.feature_queue.clone().detach(), dim=1).unsqueeze(1)])
        ...
        neg_mask = neg_accu_mask + neg_law_mask  # 罪名不同或者法律条文不同 or 只考虑罪名?
    ...
    def forward(self, fact_list, accu_label_lists, law_label_lists, term_lists):
        ...
        q, accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds = self.encoder_q(
            fact_list, accu_label_lists, law_label_lists, term_lists)  # queries: NxC
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            k, _, _, _, _, _, _, _, _, _, _ = self.encoder_k(
                fact_list, accu_label_lists, law_label_lists, term_lists)  # keys: NxC
            k = nn.functional.normalize(k, dim=1)

        # dequeue and enqueue
        self._dequeue_and_enqueue(k, accu_label_lists, law_label_lists, term_lists)
        contra_loss, _ = self._get_contra_loss(q, accu_label_lists, law_label_lists, term_lists)

        # return contra_loss, accu_loss, law_loss, term_loss, accu_preds, law_preds, term_preds
        return contra_loss, accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds

    def predict(self, fact_list, raw_fact_list, accu_label_lists, law_label_lists, term_lists, epoch_idx, name):
        # compute query features
        q, _, _, _, _, _, accu_preds, law_preds, term_preds, law_article_preds, graph_preds= self.encoder_q(
            fact_list, accu_label_lists, law_label_lists, term_lists)  # queries: NxC
        contra_loss, label_1_index = self._get_contra_loss(q, accu_label_lists, law_label_lists, term_lists)
        # if label_1_index != -1:
        #     print(
        ...
        return accu_preds, law_preds, term_preds, law_article_preds, graph_preds
