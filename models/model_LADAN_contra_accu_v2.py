...
def dynamic_partition(data, partitions, num_partitions):
    ...
    for i in range(num_partitions):
        # print("degree:", i)
        # print(f"cluster of degree {i}, {data[torch.LongTensor(partitions).eq(i).nonzero().squeeze().tolist()]}")
        if len(data[torch.LongTensor(partitions).eq(i).nonzero(as_tuple=False).squeeze().tolist()].size()) < 2:
            res.append(data[torch.LongTensor(partitions).eq(i).nonzero(as_tuple=False).squeeze().tolist()].unsqueeze(0))
        else: 
            res.append(data[torch.LongTensor(partitions).eq(i).nonzero(as_tuple=False).squeeze().tolist()])
    return res
...
class LawModel(nn.Module):
    ...
    def __init__(self, config: Config):
        ...
        self.deg_list = [len(neigh_index[i]) for i in range(config.law_label_size)]
        ...
        t = 0
        self.adj_list = [[]]
        for i in range(config.law_label_size):
            each = neigh_index[i]
            if len(each[1]) != t:
...
class MoCo(nn.Module):
    ...
    def _select_negative(self, batch_accu_labels):
        neg_mask = []
        for idx in range(batch_accu_labels.size(0)):
            cur_label = batch_accu_labels[idx]
            confused_labels = torch.nonzero(self.confused_matrix[cur_label], as_tuple=True)[1].to(batch_accu_labels.device)
            cur_mask = torch.eq(confused_labels.unsqueeze(1), self.label_queue.T) #[neg_size, queue_size]
            cur_mask = cur_mask.sum(0)
            neg_mask.append(cur_mask)
            # if batch_accu_labels[idx] == 1:
            #     print("cur label:", batch_accu_labels[idx])
            #     print("cur negative queue labels:", cur_mask.nonzero())
            #     print("queue:", self.label_queue.squeeze())
        # print("neg mask size:", torch.stack(neg_mask).size()) #[bsz, queue_size]
        return torch.stack(neg_mask)
    ...
    def forward(self, fact_list, accu_label_lists, law_label_lists, term_lists):
        ...
        q, accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds = self.encoder_q(fact_list, accu_label_lists, law_label_lists, term_lists)  # queries: NxC
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            k, _, _, _, _, _, _, _, _, _, _ = self.encoder_k(fact_list, accu_label_lists, law_label_lists, term_lists)  # keys: NxC
    ...
    def predict(self, fact_list, raw_fact_list, accu_label_lists, law_label_lists, term_lists, epoch_idx, name):
        # compute query features
        q, _, _, _, _, _, accu_preds, law_preds, term_preds, law_article_preds, graph_preds = self.encoder_q(fact_list, accu_label_lists, law_label_lists, term_lists)  # queries: NxC
