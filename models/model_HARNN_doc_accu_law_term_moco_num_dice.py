...
class LawModel(nn.Module):
    ...
    def classifier_layer(self, doc_out, accu_labels, law_labels, term_labels, money_hidden):
        ...
        num_term = torch.cat([term_rep, money_hidden], dim=-1)
        term_logits = self.term_classifier[4](self.term_classifier[3](self.term_classifier[2](num_term)))
...
class MoCo(nn.Module):
    ...
    def forward(self, legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len, money_amount_lists, drug_lists):
        ...
        accu_loss, law_loss, term_loss, accu_preds, law_preds, term_preds, q_doc_feature, q_accu_feature, q_law_feature, q_term_feature = self.encoder_q(
            legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len, money_amount_lists, drug_lists)
    ...
    def predict(self, legals, legals_len, sent_lent,accu_label_lists, law_label_lists, term_lists, money_amount_lists, drug_lists):
        # compute query features
        accu_loss, law_loss, term_loss, accu_preds, law_preds, term_preds, q_doc_feature, q_accu_feature, q_law_feature, q_term_feature = self.encoder_q(legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len, money_amount_lists, drug_lists)
