...
class LawModel(nn.Module):
    ...
    def forward(self, input_facts, accu_labels, law_labels, term_labels, input_sentences_lens, input_doc_len, money_amount_lists, drug_weight_lists):
        """
        Args:
        ...
        return accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds, accu_rep, law_rep, term_rep 
...
class MoCo(nn.Module):
    ...
    def _get_contra_loss(self, accu_query, law_query, term_query, accu_label_lists, law_label_lists, term_label_lists):
        ...
        # positive_accu_mask = accu_mask * law_mask * term_mask
        ...
        log_prob = logits - torch.log(exp_logits.sum(1, keepdim=True))
        # compute mean of log-likelihood over positive
        # positive_law_mask = accu_mask * law_mask * term_mask
    ...
    def forward(self, legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len, money_amount_lists, drug_weight_lists):
        ...
        accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds, q_accu_feature, q_law_feature, q_term_feature = self.encoder_q(
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            _, _, _, _, _, _, _, _, _, _, k_accu_feature, k_law_feature, k_term_feature = self.encoder_q(legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len, money_amount_lists, drug_weight_lists)
        ...
        return contra_accu_loss, contra_law_loss, contra_term_loss, accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds
    
    
    def predict(self, legals, legals_len, sent_lent,accu_label_lists, law_label_lists, term_lists, money_amount_lists, drug_lists):
        # compute query features
        accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds, q_accu_feature, q_law_feature, q_term_feature = self.encoder_q(legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len, money_amount_lists, drug_lists)
