...
class NeurJudge_plus(nn.Module):
    def __init__(self,config):
        ...
        # self.embs = nn.Embedding(339503, 200)
        self.embs = nn.Embedding(config.pretrain_word_embedding.shape[0], 200)
        ...
        self.time_pred = torch.nn.Sequential(
            nn.Linear(self.hidden_dim*6,256),
            torch.nn.ReLU(),
    ...
    def forward(self,charge,charge_sent_len,article,\
    article_sent_len,charge_tong2id,id2charge_tong,art2id,id2art,\
    documents,sent_lent,process,device, accu_labels, law_labels, term_labels, money_lists):
        money_hidden, _ = self.dice_model.encode_num(money_lists, money_lists)
        ...
        fact_legal_time_hidden = term_message.mean(1)
        fact_legal_time_act_hidden = self.time_pred[1](self.time_pred[0](fact_legal_time_hidden))
        fact_legal_time_hidden_num = torch.cat([fact_legal_time_act_hidden, money_hidden], -1)
        time_out = self.time_pred[4](self.time_pred[3](self.time_pred[2](fact_legal_time_hidden_num)))
        term_log_softmax = F.log_softmax(time_out, dim=-1)
        term_loss = self.term_loss(term_log_softmax, term_labels)
        _, term_predicts = torch.max(term_log_softmax, dim=1) # [batch_size * max_claims_num]

        return accu_predicts, law_predicts, term_predicts, accu_loss, law_loss, term_loss, d_hidden.mean(1), df, fact_article_hidden, fact_legal_time_hidden
...
class MoCo(nn.Module):
    def __init__(self, config):
        ...
        # create the queue
        self.register_buffer("doc_feature_queue", torch.randn(self.K, 300))
        self.register_buffer("accu_feature_queue", torch.randn(self.K, 900))
        self.register_buffer("law_feature_queue", torch.randn(self.K, 1200))
        self.register_buffer("term_feature_queue", torch.randn(self.K, 900))
    ...
    def _get_contra_loss(self, doc_query, accu_query, law_query, term_query, accu_label_lists, law_label_lists, term_label_lists):
        ...
        mean_log_prob_pos = (accu_mask * term_mask * log_prob).sum(1) / \
    ...
    def forward(self, legals, legals_len, arts, arts_sent_lent,
                ...
                sent_lent, process, device, accu_label_lists, law_label_lists, term_lists, money_lists):
        # compute query features
        accu_preds, law_preds, term_preds, accu_loss, law_loss, term_loss, q_doc_feature, q_accu_feature, q_law_feature, q_term_feature = self.encoder_q(
            legals, legals_len, arts,
            arts_sent_lent, charge_tong2id, id2charge_tong, art2id, id2art, documents, sent_lent, process, device,
            accu_label_lists, law_label_lists, term_lists, money_lists)
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            _, _, _, _, _, _, k_doc_feature, k_accu_feature, k_law_feature, k_term_feature = self.encoder_k(legals, legals_len,
    ...
    def predict(self, legals,legals_len,arts,arts_sent_lent, \
                ...
                sent_lent,process,device, accu_label_lists, law_label_lists, term_lists, money_lists):
        # compute query features
        accu_preds, law_preds, term_preds, _, _, _, _, _, _, _ = self.encoder_q(legals,legals_len,arts, \
            arts_sent_lent,charge_tong2id,id2charge_tong,art2id,id2art,documents,sent_lent,process,device, \
            accu_label_lists, law_label_lists, term_lists, money_lists)
