...
class LawModel(nn.Module):
    ...
    def forward(self, input_facts, accu_labels, law_labels, term_labels, input_sentences_lens, input_doc_len):
        ...
        accu_preds, accu_loss, law_preds, law_loss, term_preds, term_loss, law_article_preds, law_article_loss, accu_rep, law_rep, term_rep = self.classifier_layer(fact_final_rep, law_final_rep, accu_labels, law_labels, term_labels, law_article_labels)  # [batch_size, 3]
...
class MoCo(nn.Module):
    ...
    def forward(self, legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len):
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            _, _, _, _, _, _, _, _, _, _, k_accu_feature, k_law_feature, k_term_feature = self.encoder_q(legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len)
    ...
    def predict(self, legals, legals_len, sent_lent,accu_label_lists, law_label_lists, term_lists):
        # compute query features
        accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds, q_accu_feature, q_law_feature, q_term_feature = self.encoder_q(legals, accu_label_lists, law_label_lists, term_lists, sent_lent, legals_len)
