...
from models.model_LADAN_accu_law_term_moco_num_dice import MoCo
...
def evaluate(model, valid_dataloader, name, epoch_idx):
    ...
    for batch_idx, datapoint in enumerate(valid_dataloader):
        fact_list, _, accu_label_lists, law_label_lists, term_lists, money_lists, drug_lists = datapoint
        _, _, _, _, _, _, _, _, accu_preds, law_preds, term_preds, law_article_preds, graph_preds = model.forward(fact_list, accu_label_lists,law_label_lists, term_lists, config.sent_len, config.doc_len, money_lists, drug_lists)
...
def train(model, dataset, config: Config):
    ...
    for idx in range(config.HP_iteration):
        ...
        for batch_idx, datapoint in enumerate(train_dataloader):
            ...
            contra_accu_loss, contra_law_loss, contra_term_loss, accu_loss, law_loss, term_loss, law_article_loss, graph_choose_loss, accu_preds, law_preds, term_preds, law_article_preds, graph_preds = model.forward(fact_list, accu_label_lists, law_label_lists, term_lists, config.sent_len, config.doc_len, money_amount_lists, drug_lists)

            loss = accu_loss + term_loss + law_loss + graph_choose_loss + config.alpha1 * contra_accu_loss + config.alpha2 * contra_law_loss + config.alpha3 * contra_term_loss
            
            sample_loss += loss.data
            sample_accu_loss += accu_loss.data
            sample_law_loss += law_loss.data
            sample_term_loss += term_loss.data
            sample_contra_loss += contra_accu_loss.data + contra_law_loss.data + contra_term_loss.data
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savemodel', default=BASE+"/results/LADAN/LADAN_accu_law_term_moco_num_dice")
    ...
    if status == 'train':
        ...
        if os.path.exists(args.loadmodel) is False is False:
