...
def find_hard_examples(model, dev_dataloader):
    model.eval()
    ground_accu_y, ground_law_y, ground_term_y  = [], [], []
    predicts_accu_y, predicts_law_y, predicts_term_y = [], [], []

    for batch_idx, datapoint in enumerate(dev_dataloader):
        fact_list, accu_label_lists, law_label_lists, term_lists = datapoint
        _, _, _, _, accu_preds, law_preds, term_preds = model(fact_list, accu_label_lists,law_label_lists, term_lists)
    ...
    confused_matrix_accu = confusion_matrix(ground_accu_y, predicts_accu_y)#[bsz, bsz]
    confused_matrix_accu = torch.from_numpy(confused_matrix_accu).cuda()
    # print("confused matrix accu:", confused_matrix_accu[1])
    # confused_matrix_law = confusion_matrix(ground_law_y, predicts_law_y)
    # confused_matrix_term = confusion_matrix(ground_term_y, predicts_term_y)
    confused_threshold = confused_matrix_accu.sum(1) / 20 #[119]
    # print("confused threshold:", confused_threshold)
    hard_matrix = (confused_matrix_accu - confused_threshold.unsqueeze(1).repeat(1, 119))
    # print("hard matrix:", hard_matrix[1])
    hard_matrix = (hard_matrix.gt(0).float() - torch.eye(confused_matrix_accu.size(0)).cuda())
    # print("hard matrix accu:", hard_matrix[1])
    # print("Confused matirx term:", confused_matrix_term)
    return hard_matrix
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savemodel', default="/data/home/ganleilei/law/ContrastiveLJP/models/harnnContra_v3/")
    parser.add_argument('--savedset', default="/data/home/ganleilei/law/ContrastiveLJP/models/harnnContra_v3/data")
